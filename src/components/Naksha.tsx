    ...
    <section id="naksha" className="bg-ivory py-16">
      <div className="mx-auto max-w-site px-5 sm:px-8">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 md:gap-7">
          {venues.map((v) => (
            <article key={v.id} className="hairline flex flex-col overflow-hidden rounded-xl bg-cream">
              {/* Each venue shows its own footage. Wada 2 was previously a
                  text-only card next to one with a video — which quietly
                  told a visitor the second place was the lesser one. */}
              {v.poster ? (
